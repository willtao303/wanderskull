...
class Attack:
    def __init__(self, name, colour, stats, points, pxy, xy, cxy, dims):
        ...
        if self.stats["mainclass"] == "AOE":
            self.show = [
                pygame.Rect((self.cxystart[0] - 5, self.cxystart[1] - 5), (10, 10))
                for _ in range(self.stats["reach"] * self.stats["width"])
            ]
            self.attacks = [
                pygame.Rect((cxy[0] + pxy[0] - dims[1] / 2 + 20, cxy[1] + pxy[1] - dims[0] / 2 + 20), (10, 10))
                for _ in range(self.stats["reach"] * self.stats["width"])
            ]
        else:
            self.attacks = [
                pygame.Rect((xy[0] + 25, xy[1] + 25), (self.stats["h"], self.stats["w"]))
                for _ in range(self.stats["reach"] * self.stats["width"])
            ]
    ...
    def on_end(self, xy, pxy):
        if self.stats["on_end"] != "":
            # newx, newy = (
            #     self.attacks[0].x + self.dims[1] // 2 - self.xystart[0],
            #     self.attacks[0].y + self.dims[0] // 2 - self.xystart[1],
            # )
            return Attack(
                self.stats["on_end"],
                self.colour,
                weapon_library[self.stats["on_end"]],
                self.points,
                pxy,
                (xy[0], xy[1]),
                (self.show[0].x, self.show[0].y),
                self.dims,
            )
    ...
    def splash(self, pxy, xy, screen):
        ...
        self.attacks[0].x, self.attacks[0].y = (
            self.cxystart[0] + self.playerxy[0] - 600 + 25 - self.attacks[0].height / 2,
            self.cxystart[1] + self.playerxy[1] - 337 + 25 - self.attacks[0].height / 2,
        )
        self.show[0].x, self.show[0].y = (
            self.cxystart[0] - self.show[0].height / 2 - offset[0],
            self.cxystart[1] - self.show[0].height / 2 - offset[1],
        )
    ...
    def ranged_strike(self, pxy, xy, walls, screen):
        ...
        for i in range(len(self.attacks)):
            ...
            self.show[i].x = (
                self.points["chx"] * self.stats["speed"] * (self.stats["duration"] - self.dur)
                + self.xystart[0]
                - offset[0]
            )  # self.xystart[0] #- (xy[0]+25-dims[0]//2)
            self.show[i].y = (
                self.points["chy"] * self.stats["speed"] * (self.stats["duration"] - self.dur)
                + self.xystart[1]
                - offset[1]
            )  # self.xystart[1] #- (xy[1]+25-dims[1]//2)
    ...
    def sword_strike(self, pxy, xy, walls, screen):
        ...
        for j in range(self.stats["reach"]):
            for i in range(-(self.stats["width"] // 2), self.stats["width"] // 2 + self.stats["width"] % 2):  # -3,4
                ...
                if (
                    not (
                        (int((x_val + 1) / 50), int((y_val + 1) / 50)) in walls
                        or (int((x_val + 9) / 50), int((y_val + 9) / 50)) in walls
                    )
                    and col[i + self.stats["width"] // 2]
                ):
                    self.attacks[j * self.stats["width"] + i].x, self.attacks[j * self.stats["width"] + i].y = (
                        x_val,
                        y_val,
                    )
                    self.show[j * self.stats["width"] + i].x, self.show[j * self.stats["width"] + i].y = (
                        offset[0] + self.points["chx"] * (40 + j * 14) + (i * self.space[0]),
                        offset[1] + self.points["chy"] * (40 + j * 14) + (i * self.space[1]),
                    )
