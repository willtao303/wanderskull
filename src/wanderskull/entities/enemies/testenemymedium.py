...
class Enemy:
    ...
    def pathfind(self, ppos: tuple[float, float], room):
        ...
        if (x50, y50) not in room.graph:
            print("origin not found")
        while ((x50, y50) not in route) and queue:
            temp = []
            for i in queue:
                route[i] = num
            ...
            num += 1
            # print("finding...\ntemp: ", temp)
        if ((ppos50[0], ppos50[1]) not in route) or (x50, y50) not in route:
            print("path not found")
            return [(x50, y50)]
        while (ppos50[0], ppos50[1]) not in end:
            m = room.graph[end[-1]]
            for i in m:
                if i in route and route[i] == route[end[-1]] - 1:
                    end.append(i)
                    break
        for i in end:
    ...
    def goto(self, point):  # moves self.speed pixels towards point
        ...
        if self.knock_back > 0:
            chx, chy = (
                math.cos(self.knock_back_angle) * self.knock_back,
                math.sin(self.knock_back_angle) * self.knock_back,
            )
            self.moveVectorx, self.moveVectory = chx, chy
            self.knock_back -= self.knockback_total * 0.1
        elif 45 + weapon_library[self.weapon.name]["range"] < math.sqrt(
            (self.x - self.enemy_pos[0]) ** 2 + (self.y - self.enemy_pos[1]) ** 2
        ):
            self.moveVectorx, self.moveVectory = (
                int(math.cos(-math.radians(angle)) * self.speed),
                int(math.sin(-math.radians(angle)) * self.speed),
            )
        else:
    ...
    def antiwall(self, room):  # noqa: C901 TODO too complex
    ...
    def update(self, room, ppos, player):  # noqa: C901

        self.enemy_pos = [player.x, player.y]
        # check for attacks
        for nxt in player.attacks:
            idx = self.rect.collidelist(nxt.attacks)
            if (self.id not in nxt.hit) and idx != -1 and int(self.knock_back) == 0:
                if nxt.stats["mainclass"] == "AOE":
                    self.knock_back_angle = math.atan2(
                        self.rect.y - nxt.attacks[idx].y, self.rect.x - nxt.attacks[idx].x
                    )
                else:
        ...
        if not self.foundplayer:
            ...
            if not self.rect.collidepoint(self.nextpos):
                ...
                for i in range(len(self.path)):
                    if pf.distbetween((self.x, self.y), self.path[i]) / (i + 1) < dist and room.inline(
                        (self.x + 25, self.y + 25), self.path[i]
                    ):
        ...
        if (
            self.x == self.rect.x
            and self.y == self.rect.y
            and pf.distbetween((self.x, self.y), ppos) > self.range
            and pf.distbetween((self.x, self.y), self.nextpos) < 3
        ):
            pass
    ...
    def attack(self, screen, room, player, offset):
        ...
        if (
            45 + weapon_library[self.weapon.name]["range"]
            >= math.sqrt((self.x - player.x) ** 2 + (self.y - player.y) ** 2)
            and self.knock_back <= 0
        ):
                ...
                if weapon_library[self.weapon.name]["mainclass"] == "AOE":
                    ...
                    self.attacks.append(
                        Attack(
                            ...
                            (player.x, player.y),
                            (self.x, self.y),
                            (600 + random.randint(-100, 100), 337 + random.randint(-100, 100)),
                            (1200, 675),
                        )
                    )
