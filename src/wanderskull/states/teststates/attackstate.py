...
class attacker:
        ...
        self.inventory = [
            {
                "id": "sword",
                "attacklen": 15,
                "image": pygame.image.load(asset_folder + "images/temp/wepons/plat-sword.png"),
                "range": 50,
            },
            {
                "id": "spear",
                "attacklen": 30,
                "image": pygame.image.load(asset_folder + "images/temp/wepons/plat-spear.png"),
                "range": 80,
            },
            {"id": "hammer", "attacklen": 40, "image": None, "range": 80},
            {
                "id": "radial",
                "attacklen": 20,
                "image": pygame.image.load(asset_folder + "images/temp/wepons/bombs.png"),
                "range": 80,
            },
        ]
        self.slotnumber = 0
        self.itemframe = pygame.image.load(asset_folder + "images/temp/wepons/select.png")
        self.max_health = 100
        self.health = 100
    ...
    def update(self, keysdown, enemy_attacks):  # noqa: C901
        ...
        if self.knockback == 0:
            ...
            if not self.attackcounter:
                ...
                self.points["chx"], self.points["chy"] = (
                    math.cos(self.points["angle"]) * self.inventory[self.slotnumber]["range"],
                    math.sin(self.points["angle"]) * self.inventory[self.slotnumber]["range"],
                )
                ...
                if self.inventory[self.slotnumber]["id"] == "sword":
                    ...
                    for i in range(-3, 4):  # -3,4
                        self.attacks.append(
                            pygame.Rect(
                                (
                                    self.body.x + 25 + self.points["chx"] + (i * spacex),
                                    self.body.y + 25 + self.points["chy"] + (i * spacey),
                                ),
                                (10, 10),
                            )
                        )
    ...
    def draw(self, screen, h, w):
        ...
        if self.inventory[self.slotnumber]["image"] is not None:
            temp = self.inventory[self.slotnumber]["image"]
            screen.blit(
                pygame.transform.scale(temp, (temp.get_width() * 3, temp.get_height() * 3)),
                (
                    w - int(self.itemframe.get_width() / 2) - int(temp.get_width() * 1.6),
                    h - self.itemframe.get_height(),
                ),
            )
...
class dumbenemy:
    ...
    def update(self, player):
        ...
        if not self.knockback:  # if its not being knocked back
            if (
                pf.distbetween((self.rect.x, self.rect.y), (player.body.x, player.body.y)) < 100
                and self.cooldown == 0
                and self.attack_counter == 0
            ):
                ...
                spacex, spacey = math.cos(perpendicular) * 14, math.sin(perpendicular) * 14
                for i in range(-3, 4):  # -3,4
                    self.enemy_attack.append(
                        pygame.Rect(
                            (
                                self.rect.x + 25 + self.points["chx"] + (i * spacex),
                                self.rect.y + 25 + self.points["chy"] + (i * spacey),
                            ),
                            (10, 10),
                        )
                    )
    ...
    def update(self, keyspressed, keysdown):
        ...
        for i in self.enemies:
            ...
            for j in self.enemies:
                if i != j and i.rect.colliderect(j.rect):
                    i.collide = j
                    break
