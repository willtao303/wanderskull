...
class Cell:
    def __init__(self):
        ...
        # blit the image on
        self.frameimage = pygame.image.load(asset_folder + "images/temp/wepons/select.png")
        self.surf.blit(pygame.transform.scale(self.frameimage, (75, 75)), (2.5, 2.5))
        self.font = pygame.font.Font(asset_folder + "fonts/pixelmix.ttf", 15)
        self.text = ""
...
class Inventory:
    ...
    def select(self, pos):
        ...
        if cur == self.selected:
            ...
            if (
                self.inv[cur].stored is not None
                and self.inv[self.selected].stored is not None
                and self.inv[self.selected].stored.name == self.inv[cur].stored.name
                and self.inv[cur].space >= self.inv[self.selected].stored.size
            ):
    ...
    def auto_fill(self, item):
        ...
        for i in range(43):
            if (
                self.inv[i].stored is not None
                and self.inv[i].stored.name == item.name
                and self.inv[i].space >= item.size
            ):
