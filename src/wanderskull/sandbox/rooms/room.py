...
class Room:
    def __init__(self, pos, wallist, doorlist, startpoint):  # noqa: C901 TODO simplify
        ...
        for i in self.graph:
            ...
            for j in [(1, 1), (-1, 1), (-1, -1), (1, -1)]:
                new = (i[0] + j[0], i[1] + j[1])
                if new in self.graph and (i[0] + j[0], i[0]) in self.graph[i] and (i[0], i[0] + j[1]) in self.graph[i]:
                    self.graph[i].append(new)
    ...
    def render(
        self, screen, offset, render
    ):  # render = [left wall, right wall, top wall, bottom wall] or [0, width, 0 height]
        for i in self.staticwalls:
            ...
            pygame.draw.rect(
                screen, (120, 160, 160), pygame.Rect((i[0] * 50 - offset[0], i[1] * 50 - offset[1]), (50, 50))
            )
        if self.doorsclosed:
            for i in self.doors:
                pygame.draw.rect(
                    screen, (160, 82, 45), pygame.Rect((i[0] * 50 - offset[0], i[1] * 50 - offset[1]), (50, 50))
                )
    ...
    def inline(self, spos, epos):
        ...
        while math.sqrt((spos[0] - epos[0]) ** 2 + (spos[1] - epos[1]) ** 2) > n * 25:
            res.add((int((spos[0] + n * chx) / 50), int((spos[1] + n * chy) / 50)))
            n += 1
        return res.isdisjoint(set(self.walls))
